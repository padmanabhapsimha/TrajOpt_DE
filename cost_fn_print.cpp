...
std::fstream fileout("Output.txt",std::ios::out);
...
Agent_data Agent<Agent_data>::cost_fn_print(const int &selector)
{
    cost_val=0;
...
//            int NEP=params.NEP;
            ...
            double endtime=86400.0*vals.at(5);
            integrate_adaptive(controlled_stepper,rhs,x,0.0,endtime,0.1,write_cout);
...
//                    T=(params.kfactor)*((0.7119713 + 0.4089753/(r/ri) + -0.0783905/(sqr(r/ri)))/(1.0+-0.0201896*(r/ri)+0.0623406*sqr(r/ri)))/(sqr(r/ri));
            ...
            double endtime=86400.0*vals.at(5);
            integrate_adaptive(controlled_stepper,rhs,x,0.0,endtime,endtime/1000.0,write_cout);

            cost_val=sqr(abs(-1.0+((sqr(x.at(0))+sqr(x.at(1)))/sqr(rm))));///achieve position
            cost_val+=sqr(abs(-1.0+((sqr(x.at(2))+sqr(x.at(3)))/sqr(vm))));///achieve velocity
            cost_val+=sqr(abs(-1.0+sqr(abs(x.at(0)*x.at(3)-x.at(1)*x.at(2))/(rm*vm))));///achieve angular momentum
            cost_val+=sqr(x.at(9)/1e4);
//            cost_val+=(abs(mi-x.at(4))/mi);
            if(x.at(4)<0.1*mi){
                cost_val+=1e5;
            }
            break;
...
//            const double ve=sqrt(mu/re);
            ...
            /// ///////////////////////////////////////////////////////////////////////////////
            integrate_adaptive(controlled_stepper,rhs,x,0.0,endtime,1.0,write_cout);
            /// ///////////////////////////////////////////////////////////////////////////////
...
//                    }
        ...
        dxdt.at(9)=-x.at(10)*(3.0*mu*x.at(0)*rz/r4)-x.at(11)*(3.0*mu*x.at(1)*rz/r4)-x.at(12)*((-mu/r3)+(3.0*mu*x.at(2)*rz/r4));
            ...
            std::cout<<std::endl<<x0<<"\t"<<y0<<"\t"<<z0<<"\t"<<vx0<<"\t"<<vy0<<"\t"<<vz0<<"\t"<<mi;
            std::vector<Agent_data> x;
            x.push_back(x0);///X COORDINATE
            x.push_back(y0);///Y COORDINATE
            x.push_back(z0);///Z COORDINATE
            x.push_back(vx0);///X VELOCITY
            x.push_back(vy0);///Y VELOCITY
            x.push_back(vz0);///Z VELOCITY
            x.push_back(mi);///INIT MASS
            for(unsigned int i=0;i<7;i++){
                x.push_back(vals.at(i));
            }
